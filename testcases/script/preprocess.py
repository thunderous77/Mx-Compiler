...
def input_or_output(line: str):
    ...
    printStream("Input:" if is_input else "Output:")
    printStream("=== " + line.split()[0][3:] + " ===")
...
def dump(line: str):
    ...
    printStream(line[len("//#dump"):].strip())
    ...
    with open(filename) as f:
        for line in f.readlines():
            ...
            if line.startswith("//#dump"):
                dump(line)
                continue
            printStream("ignoring " + line, file=sys.stderr)
            continue
